...
export function ChartsSection({ chartData, summary, isLoading }: ChartsSectionProps) {
  ...
  return (
    <div className="w-full pb-8">
      <Card className="border-border bg-card group relative flex w-full flex-col overflow-hidden py-0 shadow-xl">
        ...
        <CardHeader className="border-border relative z-10 flex flex-col gap-4 border-b p-6 pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1 text-left">
            <div className="text-muted-foreground flex items-center gap-2">
               ...
               <span className="text-[10px] font-bold uppercase tracking-widest">Dữ liệu thời gian thực</span>
            </div>
            <CardTitle className="text-foreground text-xl font-bold tracking-tight">Thống kê hoạt động Flows</CardTitle>
            <CardDescription className="text-muted-foreground text-xs font-medium">Lượt thực thi và tương tác người dùng trong 30 ngày qua</CardDescription>
          </div>
          
          <div className="flex items-center gap-3">
            <div className="mr-4 hidden flex-col items-end lg:flex">
                <span className="text-muted-foreground text-[10px] font-bold uppercase tracking-tighter">Tổng lượt chạy</span>
            ...
            <Button variant="outline" size="sm" className="h-9 border-border px-4 text-[11px] font-bold tracking-widest uppercase shadow-sm transition-all hover:bg-muted active:scale-95">
        ...
        <CardContent className="relative z-10 flex-1 px-4 pt-6 pb-4">
          <ChartContainer config={chartConfig} className="h-60 w-full">
            <LineChart data={chartData ?? []} margin={{ left: 12, right: 30, top: 10, bottom: 0 }}>
              ...
              <XAxis
                ...
                axisLine={false}
                tickMargin={15}
                className="text-muted-foreground font-bold text-[10px] uppercase tracking-widest"
              />
              <YAxis 
                hide 
          ...
          <div className="border-border mt-6 flex items-center justify-between border-t px-2 pt-4">
            <div className="flex gap-6">
                <div className="flex items-center gap-2">
                    <div className="bg-foreground h-2 w-2 rounded-full" />
                    <span className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest">Lượt chạy</span>
                </div>
